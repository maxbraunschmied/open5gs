    ...
    udm_ue_t *udm_ue, ogs_sbi_stream_t *stream, ogs_sbi_message_t *recvmsg)
{
    ...
    ********************************** */
    ...
    if(bloom_check(&udm_ctx->expired_suci_list, udm_ue->suci, strlen(udm_ue->suci))) {
        ogs_error("[%s] SUCI found in Bloom filter! Replay attack is very likely (99.9%%)", udm_ue->suci);
    ...
    active_suci_t entry = (active_suci_t *)ogs_calloc(1, sizeof(*entry));
    if(!entry) {
    ...
    ogs_hash_set(udm_ctx->active_suci_list, entry->suci, strlen(entry), entry);
    ...
    if(!entry->timer) {
        ...
        ogs_hash_remove(udm_ctx->active_suci_list, entry->suci, strlen(entry->suci));
        ogs_free(entry);
        ogs_thread_mutex_unlock(&udm_ctx->bloom_mutex);
        return false;
    }
    ogs_timer_start(udm_ue->suci_timer, ogs_time_from_sec(60));
    ogs_debug("Started T3519 timer for SUCI [%s]", suci);
...
static void udm_timer_suci_expire(void *data)
{
    ...
    /* libbloom is not thread-safe per default: https://github.com/jvirkki/libbloom/issues/23 */
    ogs_mutex_lock(&udm_ctx->bloom_mutex);

    /* Remove the SUCI from the map of active SUCIs */
    if (ogs_hash_remove(udm_ctx->active_suci_list, entry->suci, strlen(entry->suci)) == NULL) {
        ogs_warn("SUCI [%s] could not be removed from map!", entry->suci);
    }
    ...
    ogs_debug("SUCI removed from active list and added to Bloom Filter!");

    ogs_mutex_unlock(&udm_ctx->bloom_mutex);
}