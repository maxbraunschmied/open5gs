...
#define MAX_NUM_OF_UDM_SDM_SUBSCRIPTIONS_PER_UE 4
    ...
    self.active_suci_map = ogs_hash_create(1024);
...
void udm_context_final(void)
{
    ...
    ogs_hash_do(self.active_suci_list, (ogs_hash_do_callback_fn_t) udm_remove_active_suci);
    ogs_hash_destroy(self.expired_suci_list);
...
void udm_remove_active_suci(const void *key, int klen, const void *value) {
    active_suci_t *entry = (active_suci_t *)val;
    if(!entry) return;

    if(entry->timer)
        ogs_timer_delete(entry->timer);
    if(entry->suci)
        ogs_timer_delete(entry->suci);
